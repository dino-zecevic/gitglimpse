...
[entry "label"]:
- [task "summary"] ([task "ticket" + ", " if present][task "branch" if non-empty], ~[estimated_minutes / 60 rounded to 1 decimal]h)
...
- If a task has a "ticket" field (non-null), include it before the branch: "- summary (PROJ-123, feat/auth, ~1h)".
...
- If the JSON includes a "filtered_commits" count (> 0), add a brief note after the header: "X noise commits filtered".
